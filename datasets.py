...
def loadSemanticSimilarityData(similarity_filename, split, limit=None):
    similarity_data = []
    if split == 'test':
        with open(similarity_filename, 'r') as fp:
            for record in csv.DictReader(fp, delimiter='\t'):
        ...
        with open(similarity_filename, 'r') as file_handle:
            fp = islice(file_handle, limit)
            for record in csv.DictReader(fp, delimiter='\t'):
                sent_id = record['id'].lower().strip()
                similarity_data.append((preprocess_string(record['sentence1']),
                                        preprocess_string(record['sentence2']),
...
def loadQuoraData(paraphrase_filename, split, limit=None):
    paraphrase_data = []
    if split == 'test':
        with open(paraphrase_filename, 'r') as fp:
            for record in csv.DictReader(fp, delimiter='\t'):
        ...
        with open(paraphrase_filename, 'r') as file_handle:
            fp = islice(file_handle, limit)
            for record in csv.DictReader(fp, delimiter='\t'):
                try:
                    sent_id = record['id'].lower().strip()
...
def loadSentimentAnalysisData(sentiment_filename, split, limit):
    sentiment_data = []
    num_labels = {}
    if split == 'test':
        with open(sentiment_filename, 'r') as fp:
        ...
        with open(sentiment_filename, 'r') as file_handle:
            fp = islice(file_handle, limit)
            for record in csv.DictReader(fp,delimiter = '\t'):
                sent = record['sentence'].lower().strip()
                sent_id = record['id'].lower().strip()
